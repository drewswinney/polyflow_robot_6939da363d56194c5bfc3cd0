  ...
  rosServicesToRestart, password, metadata, ... }:
{
  ...
  systemd.services.polyflow-ros-workspace = {
    ...
    environment = {
      ...
      LANG = "en_US.UTF-8";
      LC_ALL = "en_US.UTF-8";
      # Pass metadata values as environment variables
      ROBOT_ID = metadata.robotId;
      SIGNALING_URL = metadata.signalingUrl;
      PASSWORD = metadata.password;
      GITHUB_USER = metadata.githubUser;
      TURN_SERVER_URL = metadata.turnServerUrl;
      TURN_SERVER_USERNAME = metadata.turnServerUsername;
      TURN_SERVER_PASSWORD = metadata.turnServerPassword;
    };
